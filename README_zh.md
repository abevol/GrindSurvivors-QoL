...
| 模组 | 说明 |
|------|------|
| **FreshPerksFirst** | 在无限模式中，防止已拥有的技能在升级选择中出现，直到该分类组中的所有技能都被学完。 |
| **PickupRangeXpBoost** | 根据当前 `Stat.PickupRange` 相对基础值的增幅，按可配置转换比率增加获取的经验值，并在屏幕上实时显示增益百分比和额外经验值。 |
...
3. 对于 **FreshPerksFirst**：在无限模式中升级，验证已拥有的技能是否被从选择列表中过滤。
...
- 根据当前 `Stat.PickupRange` 相对基础值（360）的增幅，按可配置转换比率换算为额外经验值
